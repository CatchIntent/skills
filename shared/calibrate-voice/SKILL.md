...
Use `workspace_product_get` to see existing voice settings. Show what's currently configured (tone, personality, doNots, customInstructions, sampleResponse).
...
Ask them to pick one, then generate 2-3 sample messages in that style for their product (use `workspace_product_get` for product context) and ask: "Do these sound like you? What would you change?"
...
2. Apply via `workspace_product_update` with the `voice` field.
3. **Test it** — Use `sl_signal_search` to find a recent hot signal, then `sl_signal_generate_response` to draft a message with the new voice settings.
