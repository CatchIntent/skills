...
1. **Get plan limits** — Use `workspace_usage` to get the workspace's actual plan limits (max listeners, max keywords per listener, max signals/month). **Never hardcode plan numbers** — always use the values returned by this tool, as limits can be customized per workspace.

2. **List all listeners** — Use `sl_listener_list` to get all listeners with health metrics.

3. **Deep-dive each listener** — For each active listener, use `sl_listener_get` to pull:
   - Per-keyword performance: calls, signals, signal rate
   - Quick AI rejection rate
   - Signal conversion rate
   - Keyword health status

4. **Check dropped signals** — Use `sl_signal_dropped` (2-3 pages) to see what the AI is filtering out. Look for patterns:
...
5. **Sample recent signals** — Use `sl_signal_search` (page size 20, sorted by `createdAt` DESC) to see what's actually coming through. Note which listeners and intents generate the most value.

6. **Check content filters** — Use `sl_denylist_get` to see if any subreddits or users are already blocked. Note what's blocked so you don't re-suggest those sources.

## Phase 2: Diagnose Problems

Build a keyword performance table from the `sl_listener_get` data:
...
### Noisy Sources
If dropped signals or low-quality signals consistently come from the same subreddit or user, suggest adding them to the platform denylist using `sl_denylist_update`. Blocked sources will never generate signals.
...
1. **Understand the product** — Use `workspace_product_get` to read the workspace's brand profile (product name, description, competitors, keywords, ICP). If incomplete, ask the user and offer to update it with `workspace_product_update` so future optimizations have context. If updating ICP fields, use `workspace_icp_options` first to look up valid values for locations, industries, seniority levels, job functions, and company sizes.
...
**Use the actual plan limits from `workspace_usage`** (max listeners, max keywords per listener). Never hardcode these — they vary by plan and can be customized per workspace.
...
2. Use `sl_listener_update` to modify existing listeners
3. Use `sl_listener_create` for new ones (if they have slots)
