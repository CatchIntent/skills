...
1. **Identify target signals** — If the user specifies signal IDs, use those. Otherwise, use `sl_signal_search` to find hot/warm signals with status `new`, sorted by relevance.

2. **Get signal details** — For each signal, use `sl_signal_get` to get the full context: source post, AI reasoning, extracted signals (pain points, competitors, budget, timeline).

3. **Get brand context** — Use `workspace_product_get` to understand the product, competitors, and brand voice. This helps you evaluate draft quality and suggest the right style even before generating.

4. **Draft the response** — Use `sl_signal_generate_response` for each signal:
...
6. **Iterate** — If the user wants changes ("shorter", "more technical", "less salesy"), use `sl_signal_generate_response` again with `feedbackChips` or `freeTextFeedback`.

7. **Mark as reached out** — Once the user approves a response, offer to update the signal status to `reached_out` using `sl_signal_update_status`.
...
- If the user's product offer isn't configured (no offer in `workspace_product_get`), suggest `/craft-offer` first — a strong offer significantly improves outreach quality
