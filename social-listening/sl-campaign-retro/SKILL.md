...
1. **Workspace stats** — Use `sl_stats` with period `week` and `month` to get signal counts, won/lost, posts analyzed, noise filtered.

2. **Signal board** — Use `sl_signal_board` to see signals grouped by status (new, reached_out, replied, won, lost, no_response, ignored).

3. **Recent signals** — Use `sl_signal_search` sorted by `createdAt` DESC, page size 50, to see what's coming in. Note the mix of intents, platforms, warmth levels, and relevance scores.

4. **Dropped signals** — Use `sl_signal_dropped` (2-3 pages) to see what's being filtered out. Look for good leads being dropped vs genuine noise.

5. **Listener health** — Use `sl_listener_list` then `sl_listener_get` for each to see keyword performance, rejection rates, signal rates.

6. **Brand context** — Use `workspace_product_get` to understand the product, ICP, and brand voice settings.

7. **Usage** — Use `workspace_usage` to check plan limits and consumption.
...
**Root causes to investigate:**
- Brand voice not configured (check `workspace_product_get` voice settings)
