...
Use `sl_signal_search` with status `reached_out`, sorted by `statusChangedAt` ASC (oldest first — these have been waiting longest).
...
Use `sl_signal_get` to pull the full context: original post, AI reasoning, extracted signals, and any previous generated responses.
...
- Use `sl_signal_generate_response` in `dm` mode with `firstTouch: false` (this is a follow-up, product mention is OK)
- Keep it short — they're already interested, don't over-sell

### Question
- **Goal:** Answer thoroughly, then redirect to next step
- **Approach:** Answer their specific question with real information from `workspace_product_get`. End with a soft CTA.
...
- Use `sl_signal_generate_response` with `firstTouch: false` and `softer-tone` feedback chip
...
- Use `workspace_product_get` for competitor context
...
If they want changes, use `sl_signal_generate_response` with `firstTouch: false` and `freeTextFeedback` incorporating their notes.
...
Use `sl_signal_update_status` to apply changes.
