...
1. **Get brand context** — Use `workspace_product_get` to understand the product, competitors, and ICP. This helps you prioritize signals that mention known competitors or match target personas.

2. **Get today's signals** — Use `sl_signal_search` with warmth filter `["hot", "warm"]`, sorted by `relevanceScore` DESC, page size 20.
...
5. **Recommend next steps** — For the top 3-5 signals, suggest whether to:
   - Draft a response (offer to use `sl_signal_generate_response`)
   - Mark as reached_out if already handled

6. **Flag noisy sources** — If multiple low-relevance signals come from the same subreddit or user, mention it and offer to block them using `sl_denylist_update`. Blocked sources will never generate signals.

7. **Show stats** — End with a quick usage summary using `workspace_usage` so they know where they stand.
