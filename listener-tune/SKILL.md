...
1. **Understand the product** — Use `get_product_info` to read the workspace's brand profile (product name, description, competitors, keywords, ICP). If incomplete, ask the user and offer to update it with `update_product_info` so future optimizations have context. If updating ICP fields, use `get_icp_options` first to look up valid values for locations, industries, seniority levels, job functions, and company sizes.
