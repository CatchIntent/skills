...
1. **Resolve product context** — If the user has multiple products (`workspace_product_list`), ask which one. Otherwise default to the only product. All `li_lead_list` calls require a `productId`.

2. **Pull the priority queue** — In one call, get the actionable bucket:
   - `li_lead_list` with `productId`, `warmth: ['hot', 'warm']`, `status: ['new']`, page size 25
...
4. **Recommend a next step** — Don't just list leads. After the table, call out:
   - Top 3 hot leads with the strongest pain hypothesis (read `discovery.painHypothesis` from `li_lead_get` if needed for the top 3 only)
...
5. **Offer secondary views on demand** — Don't show these by default; mention them as follow-ups:
   ...
   - "From a specific agent?" → `agentId: ...` (call `li_agent_list` if user names one)
...
- If the user has no agents at all (`li_agent_list` returns empty), point them at `/tune-agent` (or the dashboard) to create one first.
