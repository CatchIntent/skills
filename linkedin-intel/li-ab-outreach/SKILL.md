...
1. **Resolve the lead** — `leadId` if given; otherwise `li_lead_list` `searchQuery` to look up by name and confirm the match.

2. **Load context** — Call `li_lead_get` with `productId` + `leadId`. Check:
   ...
   The product's brand voice, offer, and tone come from `workspace_product_get` if needed (the drafter pulls them automatically, but reading them helps you explain *why* a variant sounds the way it does).
...
4. **Draft each variant in parallel** — Call `li_lead_draft_outreach` 2-3 times, varying `purpose` and `painHypothesisOverride`. Each call returns body + rationale + (optional) variants.
...
6. **Get the pick** — Ask "which variant, or want to iterate?" Options:
   - **Pick one** → Show the chosen body cleanly so the user can copy-paste; offer to mark the lead as `saved` (if not already) via `li_lead_update_status`. Do **not** auto-set to `pushed` — that status is reserved for actual handoff to an outreach tool, not "I drafted it."
   - **Iterate** → Take feedback ("shorter", "less salesy", "drop the question"), redraft that one variant by calling `li_lead_draft_outreach` again with adjusted `painHypothesisOverride` or different `purpose`.
