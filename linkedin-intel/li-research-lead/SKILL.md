...
1. **Resolve the lead** — If the user provides a `leadId`, use it directly. If they give a name or LinkedIn URL, call `li_lead_list` with `searchQuery` and confirm the match before continuing. If multiple match, show the top 3 and ask.

2. **Pull the full profile** — `li_lead_get` with the resolved `leadId`. This returns:
...
3. **Check enrichment state** — If `enrichmentLevel === 'none'` and the user is about to reach out, suggest `/enrich-leads` first (or just calling `li_lead_enrich` for this one lead) — without enrichment you may be missing email and detail.
...
4. **Build the brief** — Present a 5-section block (use headings, not prose):
   ...
   - **Suggested angle** — `discovery.outreachAngle` if present; otherwise propose one grounded in their role + your product (call `workspace_product_get` for offer + voice context)
...
5. **Recommend the next move** — Pick ONE:
   ...
   - "Worth a touch but not first; mark as saved → `li_lead_update_status` to `saved`"
   - "Skip; not aligned → `li_lead_update_status` to `archived`" (only if clearly off-target)
